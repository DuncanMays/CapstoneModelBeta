...
import numpy as np
from markovSources import House, SolarPanel, WindTurbine, HydroDam
...
HydroDams = []
numHydroDams = 50
for i in range(0,numHydroDams):
    HydroDams.append(HydroDam())
    prodPriceMin += hydroPrice*HydroDams[i].min
    prodPriceMax += hydroPrice*HydroDams[i].max
...
maxGlobalProd = baseline + sum(solarPanels[i].max for i in range(0,len(solarPanels))) + sum(WindTurbines[i].max for i in range(0,len(WindTurbines))) + sum(HydroDams[i].max for i in range(0,len(HydroDams)))
minGlobalProd = baseline + sum(solarPanels[i].min for i in range(0,len(solarPanels))) + sum(WindTurbines[i].min for i in range(0,len(WindTurbines)))+ sum(HydroDams[i].min for i in range(0,len(HydroDams)))
...
hydroProduction = hydroProduction[len(T):len(hydroProduction)]
...
hydroProduction = hydroProduction[len(hydroProduction)-numDaysInView*len(T):len(hydroProduction)]
